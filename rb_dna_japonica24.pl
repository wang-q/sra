...
#----------------------------------------------------------#
my $csv_file = "japonica24.csv";

my $base_dir = shift
    || File::Spec->catdir( $ENV{HOME}, "data/japonica24" );
...
my $data_dir = {
    sra  => File::Spec->catdir( $base_dir, "SRA023116" ),
    proc => File::Spec->catdir( $base_dir, "process" ),
    bash => File::Spec->catdir( $base_dir, "bash" ),
};
my $ref_file = {
    seq => File::Spec->catfile( $base_dir, "ref", "nip_65.fa" ),
