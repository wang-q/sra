...
#----------------------------------------------------------#
my $base_dir = shift
    || File::Spec->catdir( $ENV{HOME}, "data/ath19" );
...
my $data_dir = {
    sra  => File::Spec->catdir( $base_dir, "bam" ),
    proc => File::Spec->catdir( $base_dir, "process" ),
    bash => File::Spec->catdir( $base_dir, "bash" ),
};
my $ref_file = {
    seq => File::Spec->catfile( $base_dir, "ref", "ath_65.fa" ),
    #vcf => File::Spec->catfile( $base_dir, "ref", "Dmel_65.vcf" ),
...
ITEM: for my $name (@names) {
    ...
    for (@lines) {
        ...
        my $file = File::Spec->catfile( $data_dir->{sra}, "$srr.bam" );
        if ( !-e $file ) {
            print "Can't find $srr.bam for $name\n";
            $item = undef;
            next ITEM;
        }
...
for my $item (@data) {
    ...
    $mybam->head($item);
    #$mybam->sam_dump_pe($item);
    #$mybam->bwa_aln_pe_picard($item);
    $mybam->merge_exist_bam_picard($item);
