...
#----------------------------------------------------------#
my $csv_file = "glycine_soja.csv";

my $base_dir = shift
    || File::Spec->catdir( $ENV{HOME}, "data/glycine_soja" );
...
my $data_dir = {
    sra  => File::Spec->catdir( $base_dir, "SRA009252" ),
    proc => File::Spec->catdir( $base_dir, "process" ),
    bash => File::Spec->catdir( $base_dir, "bash" ),
};
my $ref_file = {
    seq => File::Spec->catfile( $base_dir, "ref", "soybean_65.fa" ),
